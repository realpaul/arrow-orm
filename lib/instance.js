...
function Instance(model, values, skipNotFound) {
	...
	var self = this;
	['_values','_model','_dirty','_deleted','_metadata','_dirtyfields','_fieldmap','_skipNotFoundFields','_events'].forEach(function(k){
...
Instance.prototype.toJSON = function toJSON() {
	...
	_.keys(this._values).forEach(function jsonIterator(key){
		...
		if (this._skipNotFoundFields && this._skipNotFoundFields.indexOf(key)<0 && field && !field.custom && key!=='id') {
			return;
		}
		if (field.custom && field.get) {
			obj[key] = fields[key].get(this._values[key], key, this);
		}
		else {
