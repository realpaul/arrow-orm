...
MemoryConnector.prototype.findAndModify = function findAndModify(Model, options, doc, callback) {
	options.limit = 1;
	this.query(Model, options, function(err, result){
		...
		if(result && result.length){
			...
			this.save(Model, result[0], function(err, record){
				callback(err, options.new ? record : result[0]);
			});
		} else if(options.upsert){
			this.create(Model, doc, function(err, record){
				callback(err, options.new ? record : {});
			});
		} else {
