...
function Model(name, definition, skipValidation){
	...
	this.methods = definition && _.omit(definition,'name','fields','connector','metadata','mappings');
...
Model.prototype.find = function find() {
	...
	switch (arguments.length) {
		...
		case 2: {
			...
			if (_.isObject(value)) {
				return this.getConnector().find(this, value, callback);
			}
			else {
		...
		case 3: {
			return this.getConnector().find(this,arguments[0],arguments[1],arguments[2]);
		}
		default: {
 ...
 */
Model.prototype.serialize = function serialize(name, value) {
	var mapper = this.mappings[name];
	if (mapper) {
 ...
 * provide a mapping function
 */
Model.prototype.deserialize = function serialize(name, value) {
	var mapper = this.mappings[name];
	if (mapper) {
