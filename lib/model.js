...
const VALID_ACTIONS = ['create','createMany','upsert','read','findAll','findOne','findMany','findAndUpdate','count','query','distinct','update','delete','deleteAll','deleteMany'];
...
/**
 * Removes an array of model instances using a set of primary keys.
 * @param {String} ids IDs of the model to remove.
 * @param {Function} callback Callback passed an Error object (or null if successful) and the found models.
 */
Model.prototype.deleteMany =
Model.prototype.removeMany = function removeMany(ids, callback) {
	if(_.isFunction(ids)) {
		callback = ids;
		ids = [];
	}
	if(!ids || !(ids instanceof Array)){
		return callback(new Error('Non-Array type passed to deleteMany!'));
	}
	try {
		this.getConnector().deleteMany(this, ids, callback);
	} catch(e) {
		callback(e);
	}
};
...
/**
 * Finds an array of model instances using a set of primary keys.
 * @param {String} ids IDs of the model to find.
 * @param {Function} callback Callback passed an Error object (or null if successful) and the found models.
 */
Model.prototype.findMany = function findMany(ids, callback) {
	if(_.isFunction(ids)) {
		callback = ids;
		ids = [];
	}
	if(!ids || !(ids instanceof Array)){
		return callback(new Error('Non-Array type passed to findMany!'));
	}
	try {
		this.getConnector().findMany(this, ids, callback);
	} catch (E) {
		return callback(E);
	}
};
