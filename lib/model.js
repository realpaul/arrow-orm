...
function Model(name, definition, skipValidation, skipFieldValidation){
	this.name = name;
...
function validateFields(fields){
	if (!fields || !Object.keys(fields).length) {
		throw new ORMError("model must contain at least one field");
	}
...
Model.prototype.createRequest = function createRequest(request, response) {
	...
	var model = new Model(this.name, null, true, true);
	_.merge(model, this);
