...
Model.prototype.query = function query(options, callback) {
	...
	try {
		...
		if (this.getConnector().translateWhereRegex && options.where !== undefined) {
			translateLikeInQuery(options.where);
		}
...
/**
 * Looks through a query "where" for $like values that can be translated to $regex strings.
 * @param where
 */
function translateLikeInQuery(where) {
		...
		if (where.hasOwnProperty(key)) {
			...
			if (key === '$like') {
				...
				for (var i = 0; i < val.length; i++) {
					if (_.isObject(val[i])) {
						translateLikeInQuery(val[i]);
					}
				}
			}
			else if (_.isObject(val)) {
				translateLikeInQuery(val);
			}
		}
	}
}
