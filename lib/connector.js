...
function Connector(impl, config) {
	...
	// if we don't have a description, attempt to pull from pkginfo
	if (!this.description && this.pkginfo && this.pkginfo.description) {
		this.description = this.pkginfo.description;
	}
	// if we don't have a name, attempt to pull from pkginfo
	if (!this.name && this.pkginfo && this.pkginfo.name) {
