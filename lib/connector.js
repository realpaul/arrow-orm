...
function Connector(impl, config) {
	...
	var methods = impl && _.pick(impl,'create','save','update','findAndModify','findOne','findAll','find','query','delete','deleteAll','distinct','count');
	if (methods) {
...
Connector.prototype.count = function count(Model, options, callback) {
	this.query(Model, options, function(err, results){
		if (err) {
			...
			if (options.distinct) {
				var values = results.map(function(element){
					return element[options.distinct];
				});
				results = values.filter(function(element, index){
					return values.indexOf(element) === index;
				});
			}
